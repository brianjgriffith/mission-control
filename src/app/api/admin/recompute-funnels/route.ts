...
export async function POST(request: NextRequest) {
  ...
  for (const funnel of funnels) {
    ...
    for (const lc of listContacts) {
      ...
      const cid = emailToContactId.get(lc.email);
  ...
  return NextResponse.json({ message: "Recompute complete", computed, total_funnels: funnels.length });
}
