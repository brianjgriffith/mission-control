-- Fix: include charges without contact_id in sales calculations
-- Charges without contacts are treated as new deals (can't determine sequence)
...
begin
  with charge_sequence as (
    -- For charges WITH a contact: determine new vs recurring
    -- by sequencing charges per contact + product group
    select
      c.id as charge_id,
      c.amount,
      c.charge_date,
      ...
      case
        when c.contact_id is null then 1  -- no contact = treat as new
  ...
  aggregated as (
    select
      ...
      month,
      product,
      count(*) filter (where seq = 1) as deal_count,
